...
export async function query(table, columns = '*', builder = null) {
...
export function setHeader(context, title) {
  ...
  const userNode = document.querySelector('#user-name');
  if (userNode) userNode.textContent = context.profile?.display_name || context.user.email;
  const rolesNode = document.querySelector('#user-roles');
  if (rolesNode) rolesNode.textContent = context.roles.join(' · ') || 'Sin rol asignado';
}
...
export function showMessage(node, message, type = 'info') {
  if (!node) return;
