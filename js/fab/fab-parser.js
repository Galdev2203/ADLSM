...
function parseFixedWidthReaderText(source, sourceName, fallbackYear) {
  ...
  for (let index = headerIndex + 1; index < rawLines.length; index += 1) {
    ...
    if (!homeTeam || homeTeam.toUpperCase().includes('LOCAL VISITANTE')) {
...
// When Jina reads a PDF it can return the visual table as a linear list of cells:
// header -> competition separator cells -> home -> visitor -> date -> time -> venue -> ...
// This is the format of the FAB schedule PDF. Parsing each dated row from the
// previous date boundary prevents the competition separator and previous venue
// from being mistaken for team names.
function parseLinearReaderText(source, sourceName, fallbackYear) {
  ...
  const headerIndex = rawLines.findIndex(line => {
    ...
    return upper.includes('LOCAL') && upper.includes('VISITANTE') && upper.includes('FECHA') && upper.includes('HORA');
  });
  if (headerIndex < 0) return [];

  const isHeader = line => /^(LOCAL|VISITANTE|FECHA|HORA|PISTA|PISTA JUEGO)$/i.test(line);
  const dateAt = index => findDateInText(rawLines[index]);
  const timeAt = index => findTimeInText(rawLines[index]);
  let boundary = headerIndex + 1;
  let previousVenue = '';
  let section = '';

  for (let i = headerIndex + 1; i < rawLines.length; i += 1) {
    const dateMatch = dateAt(i);
    const timeSameLine = timeAt(i);
    let timeMatch = timeSameLine;
    let dateIndex = i;
    let timeIndex = i;
    if (dateMatch && !timeMatch && i + 1 < rawLines.length) {
      timeMatch = timeAt(i + 1);
      timeIndex = i + 1;
    }
    ...
    if (candidates.length >= 2) {
      homeTeam = candidates[candidates.length - 2];
      awayTeam = candidates[candidates.length - 1];
    } else {
      homeTeam = candidates[candidates.length - 1];
    }

    // Remove competition separator/header fragments from the candidate pair.
    if (COMPETITION_RE.test(homeTeam) && !COMPETITION_RE.test(awayTeam)) {
      section = homeTeam;
      homeTeam = awayTeam;
      awayTeam = '';
    }
    if (COMPETITION_RE.test(homeTeam) && COMPETITION_RE.test(awayTeam)) {
      section = `${homeTeam} ${awayTeam}`;
      continue;
    }
    if (isHeader(homeTeam) || isHeader(awayTeam)) continue;

    const venueIndex = timeIndex + 1;
    const venue = rawLines[venueIndex] || '';
...
function parseLooseReaderText(source, sourceName, fallbackYear) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const homeTeam = candidates[candidates.length - 2] || candidates[candidates.length - 1];
    const awayTeam = candidates[candidates.length - 2] ? candidates[candidates.length - 1] : '';
