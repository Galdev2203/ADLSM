...
const els = {
  ...
  url: document.querySelector('#resultsUrl'),
...
function renderSummary() {
  ...
  const cards = [
    ['Partidos', stats.matches.length],
    ['Jugados', stats.played],
    ['Victorias', stats.wins],
    ['Derrotas', stats.losses],
    ['Puntos a favor', formatNumber(stats.pointsFor)],
    ['Puntos en contra', formatNumber(stats.pointsAgainst)],
    ['Diferencia', `${stats.differential >= 0 ? '+' : ''}${formatNumber(stats.differential)}`],
...
function renderClassification() {
  ...
  if (!rows.length) {
    els.classification.innerHTML = '<div class="results-empty-small">La fuente consultada no muestra una clasificación en esta vista.</div>';
    return;
  }
...
function render() {
  ...
  els.metadata.innerHTML = `<div><span>Temporada</span><strong>${escapeHtml(dataset.season || 'No indicada')}</strong></div><div><span>Categoría</span><strong>${escapeHtml(dataset.category || 'No indicada')}</strong></div><div><span>Fuente</span><strong>${escapeHtml(dataset.url)}</strong></div>`;
  renderSummary();
...
async function load(url, teamQuery = '') {
  setStatus('Consultando FEB…', 'loading');
  els.form.querySelector('button[type="submit"]').disabled = true;
  try {
    dataset = await fetchCompetition(url);
    analysis = analyzeTeam(dataset, teamQuery);
    render();
    setStatus(`Consulta completada: ${dataset.matches.length} partidos encontrados.`, 'success');
  } catch (error) {
    dataset = null;
    analysis = null;
    ...
    setStatus(error instanceof Error ? error.message : 'No se ha podido consultar FEB.', 'error');
  } finally {
...
function exportCsv() {
  ...
  link.href = url;
  link.download = `resultados-${dataset?.season || 'feb'}.csv`;
  link.click();
...
els.form.addEventListener('submit', event => {
  ...
  load(els.url.value.trim(), els.team.value.trim());
});
els.team.addEventListener('input', () => {
  if (!dataset) return;
  analysis = analyzeTeam(dataset, els.team.value.trim());
  render();
});
els.search.addEventListener('input', renderMatches);
els.openSource.addEventListener('click', () => { if (dataset?.url) window.open(dataset.url, '_blank', 'noopener'); });
els.exportCsv.addEventListener('click', exportCsv);

els.url.value = RESULTS_PRESETS.zaragoza.url;
load(RESULTS_PRESETS.zaragoza.url);
